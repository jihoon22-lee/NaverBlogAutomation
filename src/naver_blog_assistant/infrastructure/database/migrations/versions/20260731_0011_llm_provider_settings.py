"""Allow the provider selection settings kind.

SQLite cannot alter a CHECK constraint in place, so the table is rebuilt with the wider list and the
existing rows are copied. No stored payload changes.
...
ADDED_KINDS = ("llm_providers",)


def upgrade() -> None:
    """Widen the settings kind constraint so a provider selection can be stored."""
