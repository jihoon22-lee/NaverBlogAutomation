...
- 등록 이웃의 공개 RSS와 사용자가 직접 연 검색 결과를 **글 탐색 대기열**로 모아, 한 글씩 확인할 수
  있습니다.
...
1. Toolbar action으로 Side Panel을 엽니다.
2. 필요하면 **글 탐색 대기열**에서 이웃 새 글이나 검색 후보를 열어 봅니다.
...
| 이웃 RSS·검색 후보 탐색 | 별도 secret 없음 | [글 탐색 대기열](docs/discovery.md) |
| 이메일 일일 요약 | private env file의 `DIGEST_SMTP_*` 값 | [글 탐색 대기열](docs/discovery.md#smtp-이메일-요약-선택) |

공개 RSS와 검색 결과 import는 opt-in입니다. 로그인, 쿠키 사용, 무인 페이지 순회, 댓글 자동 등록은
지원하지 않습니다.
