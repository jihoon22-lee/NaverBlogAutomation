...
local-first 도구입니다. 사용자가 약관 안내에 동의하고 선택한 글 한 건의 실행 버튼을 누른
...
6. 단계별 결과를 확인하고 **다음 대기 글 처리** 또는 **오늘의 작업으로**를 선택합니다. 자동 실행이 중단된 뒤 직접 처리했다면 **직접 처리한 단계 기록**에서 실제 완료한 단계만 선택해 대기열을 정리합니다.

수동 처리에는 복사한 댓글을 직접 붙여넣으세요. Captcha·로그인 제한·불명확한 외부 결과는 우회하거나
자동으로 다시 누르지 않습니다.
저장한 신규 이웃 검색어는 **설정 > 신규 이웃 검색어** 목록에서 삭제할 수 있으며, 이미 수집된 후보는 보존됩니다.
...
우회하지 않으며, 글별 실행 버튼 없는 무인 일괄 실행은 지원하지 않습니다.
