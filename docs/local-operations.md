...
- **공감이 `not_found`로 중단됨:** extension을 Reload한 뒤 글을 새로고침하고 다시 실행합니다.
  현재 네이버 Blog의 `u_likeit_button _face` 공감 버튼과 숨겨진 중복 control을 구분합니다. 그래도
  실패하지만 공감·댓글을 직접 완료했다면 **직접 처리한 단계 기록**에서 실제 완료한 단계를 기록해
  오늘의 작업을 정리하세요. `결과 확인 필요(unconfirmed)` 상태는 직접 완료로 바꾸지 않습니다.
