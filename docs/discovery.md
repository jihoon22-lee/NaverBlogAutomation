...
2. 다음 자동 탐색 또는 **지금 동기화**에서 공개 네이버 블로그 검색 결과를 읽습니다.
3. 후보를 검토하고 **이 글 열기** 또는 **건너뛰기**를 선택합니다.

제외어와 최신성 기간은 검색 결과 metadata를 기준으로 적용합니다. 검색 결과가 게시일을 제공하지
않는 경우에는 자동으로 오래된 글이라고 단정하지 않고 사용자가 직접 검토할 수 있도록 대기열에
남습니다.
