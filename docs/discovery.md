...
1. 웹앱의 **관리 > 설정 > 탐색 및 자동화 > 자동 탐색 설정**에서 내 블로그 ID를 저장하고
   **매일 자동으로 모으기**를 켭니다.
2. **지금 동기화** 또는 예약 시각에 공개 이웃 목록을 확인하고, 등록·활성화된 이웃의 공개 RSS에서
   최대 50개 metadata를 대기열에 넣습니다.
3. **홈**에서 수집 요약을 확인하고 **작업함**에서 이웃 새 글·신규 이웃 후보 개수와 현재 처리 중인
   글을 확인합니다.
...
완료 화면에서는 같은 출처의 **다음 대기 글 처리** 또는 **오늘의 작업으로**(작업함)를 선택할 수 있습니다.
...
2. **관리 > 설정 > 탐색 및 자동화 > 검색어·이웃·이메일 요약 관리**에서 검색어를 저장합니다. 화면에서
   현재 조정할 수 있는 항목은 검색어이며, 서버는 저장된 검색어의 제외어·최신성 규칙도 적용합니다.
   이 두 값은 API를 직접 연동할 때만 기술 필드(`excluded_terms`, `freshness_days`)로 지정할 수
   있습니다. 후보는 저장 검색어의 공백 단위 모든 단어가 **제목**에 포함될 때만 표시합니다. 예를 들어
   `코스트코 고기`는 두 단어가 모두 제목에 있어야 합니다.
...
5. **작업함 > 신규 이웃 후보**에서 후보를 검토하고 **이 글 처리하기**,
   **새 탭에서 처리** 또는 **건너뛰기**를 선택합니다.

검색 후보에서는 **관리 > 설정 > 작업 기본값 > 문구와 개인화 세부 설정**의 **서로이웃 기본 메시지**를
이번 글에 맞게 확인한 뒤
...
private env file에 아래 값을 설정하고 API를 재시작한 뒤, **관리 > 설정 > 탐색 및 자동화 >
검색어·이웃·이메일 요약 관리 > 이메일 요약**에서 **이메일로 새 글 요약 받기**를 켜세요. host,
account, sender, recipient 값은 모두 함께 제공해야 하며 password는 app password 사용을 권장합니다.
