...
- **네이버 접근 허용**은 댓글 본문 읽기와 글별 실행에만 쓰며, 한 번 허용하면 navigation 뒤에도
  유지됩니다.
- 자동 탐색·이웃 RSS 갱신·일일 요약은 local API process가 실행 중일 때만 동작합니다.

## 이웃 새 글

1. Side Panel의 **설정 > 자동 탐색 설정**에서 내 블로그 ID를 저장하고 **매일 자동 탐색**을 켭니다.
...
4. **이 글 처리하기**로 현재 탭에서 원문을 확인하거나, 현재 글을 유지하려면
   **새 탭에서 처리**를 선택합니다. 페이지 로딩이 완료되면 **댓글 작성**으로 이동해 본문
   preview를 표시합니다.
...
알림을 표시합니다. Side Panel의 **설정 > 하루 요약 알림**에서 시간대와 시각을 정하며 기본값은
`Asia/Seoul` 오전 9시입니다.
