...
내용을 최종 확인한 경우에만 별도의 사용자 승인형 교류 흐름이 동작합니다.
...
1. Side Panel의 **탐색 설정과 알림**을 열어 내 블로그 ID를 저장하고 **매일 자동 탐색**을 켭니다.
2. **지금 동기화** 또는 예약 시각에 공개 이웃 목록을 확인하고, 등록·활성화된 이웃의 공개 RSS에서
   최대 50개 metadata를 대기열에 넣습니다.
3. **이 글 열기**로 현재 탭에서 원문을 직접 확인하거나, 현재 글을 유지하려면 **새 탭 열기**를
   선택합니다. 페이지 로딩이 완료된 뒤 Side Panel이 본문 preview를 갱신합니다.
4. **건너뛰기**로 대기열에서 제외합니다.
...
2. 검색어, 제외어, 최신성 기간을 저장합니다.
3. 다음 자동 탐색 또는 **지금 동기화**에서 공식 네이버 블로그 검색 API 결과를 읽습니다.
4. 후보를 검토하고 **이 글 열기**, **새 탭 열기** 또는 **건너뛰기**를 선택합니다.

검색 후보에서는 승인 댓글과 서로이웃 신청 message를 확인한 뒤
