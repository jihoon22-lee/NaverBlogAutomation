...
OpenAI로 실제 댓글·본문을 생성하려면 PC 웹앱의 **관리 > 설정 > 연결 및 앱**에서 AI 서비스(provider)를 선택하고
...
Wi-Fi**에서 사용하려면 PC 웹앱의 **관리 > 설정 > 연결 및 앱**에서 `태블릿 연결`을 **신뢰 Wi-Fi**로
...
- **글쓰기:** 넓은 block canvas에서 문단, 소제목, 인용, 순서/비순서 목록, 구분선, 이미지·캡션을
  편집하고 임시저장합니다.
- **관리:** 이력과 일반 작업 설정을 열며, PC에서는 연결 및 앱 설정과 태블릿 연결도 관리합니다.

**관리 > 설정**은 **작업 기본값**, **탐색 및 자동화**, **연결 및 앱** 세 영역으로 나뉩니다. 자주 쓰는
기본값·동의·저장 버튼은 바로 보이고, 안전 한도·예약·AI 예산·API key·브라우저 세부 설정은 필요한
때만 접기 패널을 펼쳐 확인합니다. **데이터 관리**의 초기화는 별도 위험 영역에 접혀 있습니다.

태블릿은 작업함, 댓글, 글쓰기, 이력을 PC와 이어서 사용할 수 있습니다. API key·SMTP password·browser·LAN
연결·기기 관리는 PC 로컬 웹앱에서만 변경할 수 있습니다.

처음에는 **홈**의 `시작 준비` panel에서 준비 상태와 다음 행동을 확인합니다. blocker가 있으면
**초기 설정 계속**을 눌러 단계형 초기 설정에서 PC browser 시작·로그인이나 해당 설정을 순서대로
해결하세요. 대기열에서 글을 고른 뒤 **이 글 처리하기**를 누르면
본문 추출과 저장된 기본 AI 서비스(provider)의 댓글 후보 생성이 이어집니다.

## 5. 첫 댓글 후보 만들기

1. **관리 > 설정 > 탐색 및 자동화**에서 내 블로그 ID를 저장하고 **지금 동기화**로 대기열을 채웁니다.
2. **작업함**에서 처리할 이웃 글 또는 신규 이웃 후보를 선택합니다.
3. automation browser에서 title·본문 preview가 맞는지 확인합니다. 생성 옵션을 바꾸려면 **다시 생성**을
   누릅니다. 둘 이상의 AI 서비스(provider)가 구성된 경우에는 호출 수가 표시된 **AI 후보 비교**를 명시적으로
   선택할 수 있습니다.
...
웹앱 **관리 > 설정 > 탐색 및 자동화 > 자동 실행과 안전**에서 자동 실행 범위를 확인하고 checkbox를
...
로컬 웹앱의 **관리 > 설정 > 탐색 및 자동화**에서 내 블로그 ID(`blogId`)를 한 번 입력한 뒤
...
등록할 때 검색 API의 Blog 사용 권한도 선택해야 합니다. PC의 **관리 > 설정 > 연결 및 앱**에서
...
## 7. LLM 서비스(provider) 설정

### OpenAI

기본 fake workflow를 확인한 뒤에만 PC의 **관리 > 설정 > 연결 및 앱**에서 OpenAI 서비스(provider)와
...
PC의 **관리 > 설정 > 연결 및 앱**에서 Gemini 서비스(provider)의 key를 write-only로 저장하고 적용을 승인합니다.
...
PC의 **관리 > 설정 > 연결 및 앱**에서 Anthropic 서비스(provider)의 key를 write-only로 저장하고 적용을 승인합니다.
...
웹앱 **관리 > 설정 > 탐색 및 자동화 > 고급 · 예약 실행과 AI 예산**에서 아래 두 값을 조절합니다.

| 필드 | 의미 | 기본값 |
| --- | --- | --- |
| `daily_call_cap` | 하루 전체 LLM 호출 상한 | 60 |
| `per_request_provider_cap` | 한 요청에서 동시에 호출할 AI 서비스(provider) 수 상한 | 3 |
...
AI 서비스 API에 전송됩니다. source URL은 전송하지 않습니다. API key를 웹앱 asset, shell history,
screenshot, log 또는 commit에 남기지 마세요.

## 8. 글쓰기 워크플로

**글쓰기** 탭에서 내 글을 만드는 전체 흐름입니다. 자동 발행은 하지 않고 임시저장에서 멈추며,
...
3. **본문 생성:** 길이(`short`/`medium`/`long`), 분위기(`calm`/`warm`/`lively`),
   구성(`plain`/`sectioned`/`story`)과 AI 서비스(provider)를 고른 뒤 **본문 생성**을 누릅니다.
4. **다듬기와 자동 저장:** 결과를 확인하고 제목·block을 편집합니다. 잠시 멈추면 working copy에
   자동 저장되며, 제목이 비어 있거나 본문에 빈 문단·목록 항목이 있으면 저장할 수 없습니다. 편집 후
   **버전으로 남기기**를 눌러 현재 내용을 revision으로 확정해야 AI 다듬기와 태그 도구를 사용할 수
   있습니다. AI 생성·다듬기와 사용자가 선택한 checkpoint만 revision으로 남으므로 입력할 때마다
   이력이 불필요하게 늘어나지 않습니다. 다른 기기에서 먼저 저장했거나 최신 응답과 충돌하면 화면은
   기존 편집 내용을 덮어쓰지 않고 최신 working copy를 다시 불러오도록 안내합니다.
5. **태그 생성:** **태그 생성**으로 후보 태그를 만들고 사용할 태그를 선택합니다. 자동 저장 중이거나
   실패한 경우에는 먼저 저장을 다시 확인하고 버전으로 남깁니다. 직접 추가도 가능합니다.
...
웹앱 **관리 > 설정 > 작업 기본값**에서 기본 생성 옵션을 저장합니다.
...
| `use_image_vision` | 이미지를 AI 서비스(provider)에 전송해 분석 (vision 지원 서비스만) | `false` |
...
세션 배치와 무인 스케줄에 공통으로 적용되는 안전 정책을 **관리 > 설정 > 탐색 및 자동화 > 자동 실행과 안전**에서 저장합니다.
...
1. **관리 > 설정 > 탐색 및 자동화 > 자동 실행과 안전**에서 자동 실행에 동의
   (기술 설정 key: `automation_consent.accepted=true`)
2. 같은 화면에서 **안전 설정 저장**을 한 번 이상 실행 (기술 설정 key: `safety_policy`)
