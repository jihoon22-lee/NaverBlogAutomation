...
- **오늘의 작업:** 연결 상태, 이웃 새 글·신규 이웃 후보 수, 현재 처리 중인 글을 확인합니다.
...
5. 입력 보조만 사용한다면 네이버 입력란의 초안을 확인한 뒤 직접 등록합니다. 사용자 승인형
   자동 실행을 켰다면 아래의 건별 최종 확인을 거쳐 선택한 댓글을 등록할 수 있습니다.
...
읽거나 저장하지 않습니다. 자동 실행 동의가 꺼져 있으면 입력 보조·복사까지만 제공하며, 동의가
켜져 있어도 사용자가 글별 최종 내용을 확인하고 **이 한 건 실행**을 누른 경우에만 공감·선택 댓글
등록·선택적 서로이웃 신청을 수행합니다.
